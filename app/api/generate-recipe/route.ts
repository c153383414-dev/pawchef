...
Output JSON only (no markdown):
{
  ...
  "warnings": ["health-specific warnings if any"]
    ...
    const complianceOrder = { compliant: 0, partial: 1, 'non-compliant': 2 }
    let retryCount = 0
    while (
      ...
      try {
        ...
        if (!av.protein.ok)    failedItems.push(`protein (${Math.round(av.protein.value)}g/1000kcal, need ≥${av.protein.min}g — add more meat)`)
        ...
        if (!av.calcium.ok)    failedItems.push(`calcium (${Math.round(av.calcium.value)}mg/1000kcal, need ${av.calcium.min}–${av.calcium.max}mg — adjust calcium carbonate)`)
        if (!av.phosphorus.ok) failedItems.push(`phosphorus (${Math.round(av.phosphorus.value)}mg/1000kcal, need ≥${av.phosphorus.min}mg — add more meat)`)
        if (!av.omega3.ok)     failedItems.push(`omega-3 (${Math.round(av.omega3.value)}mg/1000kcal, need ≥${av.omega3.min}mg — include fish oil)`)
        ...
        const complianceHint = failedItems.length > 0
          ? `\n\nCRITICAL CORRECTION NEEDED (attempt ${retryCount}): The previous recipe failed AAFCO standards:\n${failedItems.map(f => `- ${f}`).join('\n')}\nYou MUST fix ALL of these to produce a compliant recipe.`
          : ''

        const cRetry = await openai.chat.completions.create({
          model,
          messages: [{ role: 'user', content: prompt + complianceHint }],
        ...
        if (complianceOrder[cValidation.complianceLabel] <= complianceOrder[validation.complianceLabel]) {
