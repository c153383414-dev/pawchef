      ...
      <div style={{ background: '#FDFAF5', borderRadius: 20, width: '100%', maxWidth: 400, padding: 36, position: 'relative' }}>
        ...
        {!showReset ? (
          <>
            ...
            {msg && <div style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 12, fontSize: 13, background: msg.type === 'success' ? '#EBF2EC' : msg.type === 'error' ? '#FAE8E8' : '#FBF0E4', color: msg.type === 'success' ? '#7A9E7E' : msg.type === 'error' ? '#C45C5C' : '#C8813A' }}>{msg.text}</div>}
            ...
            <div style={{ marginBottom: activeTab === 'signup' ? 16 : 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                {activeTab === 'login' && <span onClick={() => { setShowReset(true); setMsg(null) }} style={{ fontSize: 12, color: '#7A9E7E', cursor: 'pointer' }}>忘记密码？</span>}
              </div>
            ...
            <button onClick={handleAuth} disabled={loading} style={{ width: '100%', padding: 12, borderRadius: 10, background: '#1C1A16', color: '#FDFAF5', fontSize: 15, fontWeight: 500, border: 'none', cursor: loading ? 'wait' : 'pointer', marginTop: 8, opacity: loading ? 0.7 : 1, fontFamily: 'inherit' }}>
              {loading ? '处理中…' : activeTab === 'login' ? '登录' : '免费注册'}
            </button>
            <p style={{ fontSize: 12, color: 'rgba(28,26,22,0.3)', textAlign: 'center', marginTop: 16 }}>继续即表示你同意我们的服务条款和隐私政策</p>
          </>
        ) : (
          <>
            ...
            {msg && <div style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 12, fontSize: 13, background: msg.type === 'success' ? '#EBF2EC' : '#FAE8E8', color: msg.type === 'success' ? '#7A9E7E' : '#C45C5C' }}>{msg.text}</div>}
            <label style={labelStyle}>注册邮箱</label>
            <input style={{ ...inputStyle, marginBottom: 8 }} type="email" placeholder="your@email.com" value={resetEmail} onChange={e => setResetEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleReset()} />
            ...
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              <span onClick={() => { setShowReset(false); setMsg(null) }} style={{ fontSize: 13, color: '#7A9E7E', cursor: 'pointer' }}>← 返回登录</span>
            </div>
          </>
        )}
      </div>
